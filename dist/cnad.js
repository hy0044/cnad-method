...
function validateAgentsIntegrationTarget() {
    ...
    if (state === 'missing')
        accessSync(agentsPath, constants.W_OK);
}
...
function validateManifest(value) {
    if (!isRecord(value))
        throw new Error('Invalid CNAD manifest: expected an object.');
    ...
    if (files == null)
        return validatedVersion === undefined ? {} : { version: validatedVersion };
    if (!isRecord(files)) {
...
function readManifest() {
    ...
    if (!info.isFile())
        throw new Error('Invalid CNAD manifest: `.cnad/version.json` must be a regular file.');
    if (info.nlink > 1)
        throw new Error('Invalid CNAD manifest: `.cnad/version.json` must not have multiple hard links.');
...
function init() {
    ...
    for (const entry of entries)
        assertSafeRepositoryPath(managedTarget(entry.rel));
...
function inspectUpdate() {
    ...
    for (const [managedPath, recordedHash] of Object.entries(manifest.files ?? {})) {
        ...
        if (hash(readFileSync(target, 'utf8')) !== recordedHash)
    ...
    for (const managedPath of Object.keys(manifest.files ?? {})) {
        if (!nextPaths.has(managedPath))
            changes.push(`${managedPath} will be removed`);
    }
    for (const entry of entries) {
        ...
        if (!previousHash) {
            if (existsSync(target))
                conflicts.push(`${managedPath} exists but is not CNAD-owned`);
...
function preflightUpdateMutations(manifest, entries) {
    ...
    if (!manifestMatchesEntries(manifest, entries))
        existingTargets.push(manifestPath);
...
function main() {
    ...
    if (command === 'init' && args.length === 0)
        return init();
    if (command === 'update' && args.length === 1 && args[0] === '--check')
        return checkUpdate();
